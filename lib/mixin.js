...
export async function asyncData({ app, store, params, error }) {
  ...
  const locale = store.state.i18n ? store.state.i18n.locale : "en";

  let entity;
  let markdown;

  try {
    entity = store.state[entitiesName][entitiesName].find(e => e.slug === entityValue && e.language === locale);
  } catch (e) {
    error({ statusCode: 404, message: `That ${entityName} does not exist!` });
  }

  try {
    if (store.state.i18n && store.state.i18n.locale) {
      markdown = await app.loadMarkdown(`${entitiesName}/${entityValue}/${entityValue}-${store.state.i18n.locale}.md`);
    }
  } catch (e) {
    try {
      markdown = await app.loadMarkdown(`${entitiesName}/${entityValue}/${entityValue}.md`);
    } catch (e) {
      return error({ statusCode: 500, message: `Something went wrong!` });
    }
  }

  markdown.attributes = merge(markdown.attributes, entity);
...
*/
export function getEntities(entityName, storeName) {
  return function({ store }) {
    const locale = store.state.i18n ? store.state.i18n.locale : "en";

    storeName = storeName || entityName;

    return {
      [entityName]: store.state[storeName][entityName].filter(e => e.language === locale)
    };
  };
}
