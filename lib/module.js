...
export function writeJSONFiles(processed, source) {
  ...
  */
  for (const [entityName, { entities, relationships }] of Object.entries(processed)) {
    // console.log("Writing " + entityName + ".json");
    writeFileSync(getJsonPath(entityName + ".json"), JSON.stringify(entities));

    for (const [relationshipName, data] of Object.entries(relationships)) {
      // console.log("Writing " + relationshipName + ".json");
      writeFileSync(getJsonPath(relationshipName + ".json"), JSON.stringify(data));
    }
  ...
  */
  ...
  for (const [relationshipName, data] of Object.entries(relationshipData)) {
    // console.log("Writing " + entityName + "_" + relationshipName + ".json");
    writeFileSync(getJsonPath(entityName + "_" + relationshipName + ".json"), JSON.stringify(data));
  }
}
