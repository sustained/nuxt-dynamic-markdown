...
export default async function DynamicMarkdown(options) {
  ...
  */
  ...
  /*
    Watch for changes!
  */
  if (!this.options.build.watch) this.options.build.watch = [];
  this.options.build.watch = this.options.build.watch.concat(["~/contents"]);
  ...
  */
  this.extendBuild(function(config, { isDev, isClient, isServer, loaders }) {
    ...
    /*
      NOTE: Necessary because we do this locally to ease development in the example project:

        rm -rf ./node_modules/nuxt-dynamic-markdown && ln -s ~/Projects/nuxt-dynamic-markdown ./node_modules/nuxt-dynamic-markdown
  ...
  */
  ...
  this.nuxt.addHooks({
    build: {
      async before() {
        console.log("ℹ [nuxt-dynamic-markdown] Processing sources (build:before).");
        await processSources(options);
        console.log("✔ [nuxt-dynamic-markdown] Complete!");
      }

      /*
        TODO: Figure out some way to see what changed. We have getChangedFiles(compiler) 
              but it doesn't seem to work and I don't know why.

        NOTE: At present uncommenting the code below will cause infinite loops due to the 
              ~/static/json directory triggering a recompile when the JSON files are written 
              to it. This occurs even if/when we add ~/static/json to .nuxtignore. Bug?
      */
      // async compiled({ name, compiler, stats }) {
      //   console.log("ℹ [nuxt-dynamic-markdown] Reprocessing sources (build:compiled).");
      //   await processSources(options);
      //   console.log("✔ [nuxt-dynamic-markdown] Complete!");
      // }
    },
    ...
    generate: {
      async before() {
        console.log("ℹ [nuxt-dynamic-markdown] Processing sources (generate:before).");
        routes = await processSources(options);
        console.log("✔ [nuxt-dynamic-markdown] Complete!");
      },

      extendRoutes(routes) {
        console.log("ℹ [nuxt-dynamic-markdown] Extending routes (generate:extendRoutes).");
...
*/
export async function processSources(options) {
  const routes = [];
...
export function getChangedFiles(compiler) {
